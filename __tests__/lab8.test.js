describe('Basic user flow for Website', () => {
  ...
  it('Initial Home Page - Check for 20 product items', async () => {
    ...
    // eg: page.$('product-item') = document.querySelector('product-item);
    // $ = querySelector
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let cartCountVal = await cartCount.getProperty('innerText');
    expect(await cartCountVal.jsonValue()).toBe("20");
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItems = await page.waitForSelector('product-item');

    for (let i = 0; i < prodItems.length; i++) {
      let shadowroot = await prodItems[i].getProperty('shadowRoot');
      let button = await shadowroot.waitForSelector('button');
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    let cartCount = await page.$('#cart-count');
    let cartCountVal = await cartCount.getProperty('innerText');
    expect(await cartCountVal.jsonValue()).toBe("0");
